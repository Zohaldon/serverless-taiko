...
Super.prototype.clickAndWaitForRequest = function (pattern, options) {
    ...
    let promises = [
        this._page.waitForRequest(callback, options),
        this.click(),
    ];
    return Promise.all(promises).then((value) => value.shift());
};
Super.prototype.clickAndWaitForResponse = function (pattern, options) {
    let callback = (request) => {
        let url = request.url();
        if (typeof pattern === 'string' && pattern.includes('*') === true) {
            pattern = new RegExp(pattern.replace(/[-\/\\^$+?.()|[\]{}]/g, '\\$&').replace(/[*]+/g, '.*?'), 'g');
    ...
    let promises = [
        this._page.waitForResponse(callback, options),
        this.click(),
    ];
    return Promise.all(promises).then((value) => value.shift());
};
